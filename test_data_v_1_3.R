...
# Script for generating the features from the test data
library(entropy)
bidder<-read.csv(file="test.csv",header=TRUE)
...
mean_bids_auction,sd_bids_auction,median_bids_auction,max_bids_auction,uniqueCountries,uq_mean_unique_country,uq_sd_unique_country,uq_max_unique_country)
write.csv(file="test_sample_sub5.csv",data,row.names=FALSE)
